jQuery(document).ready(function(){
	...
	(function($, wpzoomRatingStars) {
	    var WPZOOM_Rating_Star;

	    WPZOOM_Rating_Star = (function() {
	        WPZOOM_Rating_Star.prototype.defaults = {
	            rating: void 0,
	            rating_total: void 0,
	            numStars: 5,
	        ...
	        function WPZOOM_Rating_Star($el, options) {
	            var i, _, _ref,
	                _this = this;
	            ...
	            this.$el.next().find('small.wpzoom-rating-average').html( this.options.rating );
	            this.$el.next().find('small.wpzoom-rating-total-votes').html( this.options.rating_total );
	            this.syncRating();
	        ...
	        WPZOOM_Rating_Star.prototype.setRating = function(rating, element) {

	        	// prevent user multiple votes with same rating value
	            if ( wpzoomRatingStars.user_rated && parseInt(this.options.rating) === rating ) {
	                return false;
	            }
	        ...
	        WPZOOM_Rating_Star.prototype.syncRating = function(rating) {
	            var i, _i, _j, _ref;
	            ...
	            if (rating) {
	                for (i = _i = 0, _ref = rating; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
	                	this.$el.find('li').eq(i).removeClass('fa-star-o').addClass('fa-star');
	                }
	            }
	            if (rating && rating < 5) {
	                for (i = _j = rating; rating <= 4 ? _j <= 4 : _j >= 4; i = rating <= 4 ? ++_j : --_j) {
	                    this.$el.find('li').eq(i).removeClass('fa-star').addClass('fa-star-o');
	                }
	            }
	            if (!rating) {
	                return this.$el.find('li').removeClass('fa-star').addClass('fa-star-o');
	            }
	        };

	        return WPZOOM_Rating_Star;
	    ...
	    return $.fn.extend({
	        starrr: function() {
	            ...
	            return this.each(function() {
	                ...
	                if (!data) {
	                    $(this).data('star-rating', (data = new WPZOOM_Rating_Star($(this), option)));
	                }
