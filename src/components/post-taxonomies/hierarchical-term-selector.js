...
class HierarchicalTermSelector extends Component {
		...
		findOrCreatePromise
			.then( ( term ) => {
				...
				const termAddedMessage = sprintf(
					// translators: %s: Term
					_x( '%s added', 'term', 'wpzoom-recipe-card' ),
					get(
						this.props.taxonomy,
	...
	setFilterValue( event ) {
		...
		const resultsFoundMessage = sprintf(
			// translators: %d: The results found singular & plural
			_n( '%d result found.', '%d results found.', resultCount, 'wpzoom-recipe-card' ),
			resultCount
		);
	...
	render() {
		...
		const filterLabel = sprintf(
			...
			_x( 'Search %s', 'term', 'wpzoom-recipe-card' ),
			get(
				this.props.taxonomy,
				[ 'name' ],
		...
		const groupLabel = sprintf(
			// translators: %: Available term
			_x( 'Available %s', 'term', 'wpzoom-recipe-card' ),
			get(
				this.props.taxonomy,
				[ 'name' ],
				slug === 'category' ? __( 'Categories' ) : __( 'Terms' )
