...
class WPZOOM_Settings_Fields {
	...
	/**
	 * @since 2.2.0
	 * @var array $license_options  Store all license options value of the setting we've registered with register_setting()
	 */
	public static $license_options;

	/**
	 * @since 2.2.0
	 * @var array $whitelist_license  List with available license id's
	 */
	public static $whitelist_license;
	...
	public function __construct() {
		...
		self::$license_options = WPZOOM_Settings::$license_options;
		self::$whitelist_license = array( 'wpzoom_rcb_plugin_license_key', 'wpzoom_rcb_plugin_license_status' );
	...
	?>
		<fieldset class="wpzoom-rcb-field-input">
			...
			<input name="<?php echo $option_name.'['. esc_attr( $args['label_for'] ) .']'; ?>" type="<?php echo esc_attr( $type ) ?>" id="<?php echo esc_attr( $args['label_for'] ) ?>" value="<?php echo $value ?>" class="regular-text" <?php echo ( self::is_disabled( $args ) ? 'disabled' : '' ); ?>/>
	...
	?>
		<fieldset class="wpzoom-rcb-field-checkbox">
			...
			<label for="<?php echo esc_attr( $args['label_for'] ) ?>">
				<input type="hidden" name="<?php echo $option_name.'['. esc_attr( $args['label_for'] ) .']'; ?>" value="0" />
	...
	?>
		<fieldset class="wpzoom-rcb-field-select">
			...
			<select id="<?php echo esc_attr( $args['label_for'] ); ?>"
				name="<?php echo $option_name.'['. esc_attr( $args['label_for'] ) .']'; ?>"
	...
	public static function parse_text_field( $args ) {
		$default_value = WPZOOM_Settings::get_default_option_value( $args['label_for'] );
		$options = self::is_license_field( $args ) ? self::$license_options : self::$options;
		...
		if ( self::is_disabled( $args ) ) {
			...
			WPZOOM_Settings::update_option( $options );
	...
	/**
	 * Parse option name for field name.
	 * 
	 * @since 2.2.0
	 * @param array $args  The field arguments
	 * @return boolean|string
	 */
	public static function parse_option_name( $args ) {
		return self::is_license_field( $args ) ? 'wpzoom-recipe-card-license' : 'wpzoom-recipe-card-settings';
	}
	...
	/**
	 * Check is license field
	 * 
	 * @since 2.2.0
	 * @param array $args Field arguments
	 * @return boolean
	 */
	public static function is_license_field( $args ) {
		return in_array( $args['label_for'], self::$whitelist_license );
	}
